# Credituz OS Growth — análise, cobrança e crédito multibanco

> Credituz OS Growth é o plano do Credituz OS para empresas que crescem: 50 análises de crédito, até 600 cobranças e 10 usuários por mês, mais embedded credit para simular, solicitar e acompanhar crédito multibanco dentro da própria jornada do cliente. Antes vendido como produto separado sob o nome CORBAN AI.

- Canonical: https://credituz.ai/pages/corban-ai.html
- Plano: Credituz OS Growth
- Preço: R$ 697 por mês (análise adicional: R$ 16)
- Categoria: análise de crédito, CRM de crédito, cobrança com IA, embedded credit e CRM Corban
- Disponibilidade: Brasil
- Atualizado: 2026-09

## Resposta direta

O Growth inclui tudo do Credituz OS Start e organiza o ciclo **Cliente → Análise → Simulação → Instituições → Pedido → CRM Corban → Contratação**. A decisão final, a aprovação e a concessão do crédito continuam sendo realizadas pelas instituições financeiras parceiras.

## O que inclui

- 50 análises de crédito por mês (adicional: R$ 16; análises não usadas não acumulam);
- até 600 cobranças por mês, cada uma como régua com até 3 datas em WhatsApp e e-mail;
- até 10 usuários;
- análise e política de crédito, CRM de crédito, recebíveis e analytics;
- simulação de crédito multibanco;
- pedido e acompanhamento de crédito;
- CRM Corban;
- organização de documentos e pendências;
- acompanhamento de bancos, status e contratação;
- jornada de embedded credit integrada à experiência da empresa.

## Diferença para o Start

| | Start | Growth |
| --- | --- | --- |
| Mensalidade | R$ 297 | R$ 697 |
| Análises incluídas | 15/mês | 50/mês |
| Análise adicional | R$ 20 | R$ 16 |
| Cobranças | até 200/mês | até 600/mês |
| Usuários | 3 | 10 |
| Embedded credit (simulação multibanco, pedido, CRM Corban) | não | sim |

## Para quem foi criado

- imobiliárias que desejam acompanhar o financiamento depois da venda;
- construtoras e incorporadoras que precisam enxergar a operação até o repasse;
- correspondentes e originadores que gerenciam pedidos, documentos, instituições e etapas.

## O que é um pipeline de crédito?

Pipeline de crédito é a visão das operações desde a simulação até a contratação. Ele mostra etapa, instituição, documentos, pendências, responsável, próxima ação e status de cada pedido. No Growth, esse pipeline fica no CRM Corban para que comercial e operação trabalhem sobre o mesmo contexto.

Para análise, política interna, CRM de crédito e cobrança com volume menor, o [Credituz OS Start](https://credituz.ai/pages/credituz-os.html) atende. Para volumes e módulos customizados, o [Enterprise](https://credituz.ai/pages/enterprise.html).

## Indicadores divulgados

- R$ 300 milhões em crédito solicitado pela plataforma;
- mais de 40 operações usando a infraestrutura Credituz.

## Links

- Plano Growth: https://credituz.ai/pages/corban-ai.html
- Credituz OS: https://credituz.ai/pages/credituz-os.html
- Enterprise: https://credituz.ai/pages/enterprise.html
- Contato: https://wa.me/5511936209409
