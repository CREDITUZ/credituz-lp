...
- `docs/` guarda a documentação interna (este arquivo, design system, deploy). Fica fora da raiz de propósito: o GitHub Pages publica o repositório inteiro, e o `robots.txt` bloqueia `/docs/`, `/scripts/`, `/CLAUDE.md` e `/README.md` para não misturar documentação com a landing page nos buscadores e nos crawlers de IA. O `CLAUDE.md` da raiz é só um ponteiro (`@docs/CLAUDE.md`), porque a ferramenta procura o arquivo nesse caminho.
